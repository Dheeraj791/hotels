...
if(isset($db) && $db !== false){
    ...
    $query_article .= "
    AND (publish_date IS NULL || publish_date <= ".time().") AND (unpublish_date IS NULL || unpublish_date > ".time().") AND lang = ".LANG_ID."
    ...
    foreach($result_article as $i => $row){
        ...
        $html .= "
        <article class=\"blog-article col-sm-12 mb20".$tags."\" itemprop=\"blogPosts\" itemscope itemtype=\"http://schema.org/BlogPosting\">
            ...
            <div class=\"mt10 labels\" dir=\"ltr\">
                ...
                $html .= "<span class=\"label label-default mb5\"><i class=\"fas fa-fw fa-comment\"></i> ".$nb_comments." ".mb_strtolower($texts['COMMENTS'], "UTF-8")."</span>";
                
                if($result_users->execute() != false){
                    foreach($result_users as $user_article)
                        $html .= "<span class=\"label label-default mb5\"><i class=\"fas fa-fw fa-user\"></i> <span itemprop=\"creator author publisher\">".$user_article['login']."</span></span>";
                }
                ...
                if($result_tag !== false){
                    ...
                    if($nb_tags > 0){
                        foreach($result_tag as $i => $row){
                            ...
                            $html .= "<span class=\"label label-default mb5\"><i class=\"fas fa-fw fa-tag\"></i> ".$tag_value."</span>";
                        }
                    }
                }
                $html .= "
