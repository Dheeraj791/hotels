...
if(!ADMIN && (MAINTENANCE_MODE == 0  || (isset($_SESSION['user']) && ($_SESSION['user']['type'] == 'administrator' || $_SESSION['user']['type'] == 'manager')))){
    ...
    $sys_pages = array();
    $parents = array();
    $result_page = $db->query('SELECT * FROM pm_page WHERE (checked = 1 OR checked = 0) AND lang = '.$id_lang.' ORDER BY rank');
    ...
    $articles = array();
    $result_article = $db->query('SELECT * FROM pm_article WHERE id_page IN('.implode(',', array_keys($pages)).') AND (checked = 1 OR checked = 3) AND (publish_date IS NULL || publish_date <= '.time().') AND (unpublish_date IS NULL || unpublish_date > '.time().') AND lang = '.$id_lang.' ORDER BY CASE WHEN publish_date IS NOT NULL THEN publish_date ELSE add_date END DESC');
